...
本仓库中的私有电力系统数据集用于支持论文中的事件条件残差算子学习实验。该部分数据围绕电力系统中的两类典型预测任务构建：一类是用户侧用电量预测，包括居民用电量数据集（REC）和农业用电量数据集（AEC）；另一类是电力市场价格预测，包括日前电价数据集（DEP）和实时电价数据集（REP）。这些数据集均来源于某省电力系统真实业务记录，能够反映用户侧用电需求、电力市场价格以及外部事件扰动下的非平稳变化特征。与此同时，我们针对上述时间序列构建了对应的事件文本数据集，用于刻画政策调控、经济活动、极端天气、节假日和市场运行状态等外部因素对电力系统未来演化的影响。
...
- **数据来源与采集：** REC 和 AEC 分别来源于某省电力公司居民侧和农业侧用电统计记录，并结合气象部门公开温度数据进行整理。两个数据集覆盖 2023 年 1 月 1 日至 2025 年 12 月 31 日，以 1 天为采样频率记录，共计 1096 条日级样本。REC 用于刻画居民用户每日用电需求变化，AEC 用于刻画农业生产相关用电需求变化。

- **数据特征：** REC 和 AEC 均为日级单变量时间序列，每条记录对应一个自然日及其用电量观测值。核心字段包括 `date`、`electricity_consumption` 和 `temperature`，其中 `date` 表示日期，`electricity_consumption` 表示对应日期的用电量，`temperature` 表示公开气象数据中的温度信息。REC 能够反映居民用电在季节变化、节假日、气温变化和生活行为变化下的波动规律；AEC 能够反映农业用电在季节性生产周期、天气条件、灌溉需求、农事活动和相关政策影响下的变化规律。

- **数据处理与质量控制：** REC 和 AEC 经过统一的数据预处理流程。首先，对原始日期字段进行标准化处理，并按照自然日构建连续时间索引；其次，对时间序列完整性、重复日期和明显异常记录进行检查，确保每个自然日仅保留一个有效用电量观测值。对于个别异常用电波动，不进行机械删除，而是结合节假日、极端天气、温度变化和业务背景进行判断。若异常波动具有合理现实含义，则予以保留，用于反映真实用电需求中的非平稳变化。
...
由于原始数据涉及真实电力系统运行信息、电力市场交易记录和用户侧用电统计数据，出于数据安全、隐私保护和业务合规考虑，完整原始数据不直接公开。为支持学术研究与结果复现，研究人员可通过联系通讯作者申请获取去识别化样本数据。获批数据仅限于学术研究、方法验证和重复性实验使用，不得用于商业用途、公开传播或未经授权的二次分发。
