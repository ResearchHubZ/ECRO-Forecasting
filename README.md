...
**1. DEP and REP: Electricity Price Datasets**

- **数据来源与采集。**  
DEP 和 REP 分别来源于某省日前电价记录和实时电价记录，均由电力交易相关业务系统生成。两个数据集覆盖 2024 年 1 月 1 日至 2025 年 6 月 15 日，以 1 小时为采样频率记录，共计 12624 个小时级样本。DEP 用于反映日前交易环节形成的价格变化，REP 用于反映实际运行过程中受供需状态、市场交易、负荷波动和运行约束影响形成的实时价格变化。

- **数据特征。**  
DEP 和 REP 均为小时级单变量时间序列，每条记录对应一个小时级时间戳及其电价观测值。核心字段包括 `timestamp` 和 `price`，其中 `timestamp` 表示小时级时间戳，`price` 表示对应时刻的电价。两个数据集覆盖工作日、周末、节假日、季节转换期和电力需求高峰期，能够反映电力市场价格的日内波动、周周期变化、季节性变化以及异常市场波动。

- **数据处理与质量控制。**  
DEP 和 REP 经过统一的数据预处理流程。首先，对原始时间字段进行标准化处理，并按照小时频率构建连续时间索引；其次，对重复时间戳、缺失小时和异常记录进行检查，确保每个小时仅对应一个有效电价观测值。对于极端高价或低价点，不进行简单删除，而是结合电力市场运行特征进行核查。若异常波动具有合理市场含义，例如供需紧张、交易波动、节假日负荷变化或极端天气影响，则予以保留，用于反映电力市场中的非平稳变化。
